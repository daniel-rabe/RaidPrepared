...
local function ExpandFlyout(flyoutID, fn, lineName)
    ...
    for slot = 1, numSlots do
        ...
        if okSlot and spellID then
            ...
            -- and seeing them in /fck travel scan is how teleport IDs get harvested.
...
function Collector.Audit()
    ...
    if bad == 0 then
        ...
        T:Out("audit: %d of %d did not resolve. Item data is fetched from the server on demand - wait a few seconds and run |cffffff00/fck travel audit|r again; whatever still fails is a bad ID.", bad, total)
    end
...
function Collector.ReportDiscovery()
    ...
    if count == 0 then
        ...
        T:Out("Run |cffffff00/fck travel scan <part of the port name>|r to find what it is actually called.")
    end
end
